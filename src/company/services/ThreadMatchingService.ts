...
export class ThreadMatchingService {
  ...
  async matchEmailToRFQ(emailReply: any): Promise<ThreadMatchResult | null> {
    try {
      ...
      if (messageIdMatch && messageIdMatch.confidence > 0.8) {
        logger.info(
          `Matched by Message-ID with confidence ${messageIdMatch.confidence}`
        );
      ...
      if (subjectMatch && subjectMatch.confidence > 0.7) {
        logger.info(
          `Matched by subject with confidence ${subjectMatch.confidence}`
        );
        return subjectMatch;
      }
      ...
      if (emailMatch && emailMatch.confidence > 0.6) {
        logger.info(
          `Matched by email address with confidence ${emailMatch.confidence}`
        );
        return emailMatch;
      }
      ...
      if (contentMatch && contentMatch.confidence > 0.5) {
        logger.info(
          `Matched by content with confidence ${contentMatch.confidence}`
        );
        return contentMatch;
      }
   ...
   */
  private async matchByMessageId(
    emailReply: any
    ...
    try {
      ...
      if (rfqNumberMatch) {
        ...
        if (rfq) {
          ...
          const matchingRecipient = rfq.recipients.find(
            (r) =>
              r.contact.email.toLowerCase() ===
              emailReply.fromEmail.toLowerCase()
          );
      ...
      if (subject.startsWith("re:")) {
        ...
        if (recentRFQ) {
          const matchingRecipient = recentRFQ.recipients.find(
            (r) =>
              r.contact.email.toLowerCase() ===
              emailReply.fromEmail.toLowerCase()
          );
   ...
   */
  private async matchByEmailAddress(
    emailReply: any
  ): Promise<ThreadMatchResult | null> {
    try {
