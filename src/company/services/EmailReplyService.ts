...
export class EmailReplyService {
  ...
  async processIncomingEmail(
    ...
    try {
      ...
      const threadMatch = await this.threadMatchingService.matchEmailToRFQ(
        emailReply
      );
      ...
      // Perform AI parsing
      const parsingResults = await this.aiParsingService.parseEmailContent(
        emailReply
      );
      ...
      const requiresReview =
        confidenceScore < 0.7 ||
        parsingResults.some((r) => r.confidenceScore < 0.6);
      ...
      logger.info(
        `Successfully processed email ${emailData.messageId} with confidence score ${confidenceScore}`
      );
  ...
  private calculateOverallConfidence(parsingResults: any[]): number {
    ...
    const totalScore = parsingResults.reduce(
  ...
  async getEmailReply(
    replyId: string
  ): Promise<EmailReplyWithRelations | null> {
   ...
   */
  async getRepliesRequiringReview(
    companyId: string,
    options: {
      page?: number;
      limit?: number;
  ...
  async getEmailReplyStats(companyId: string): Promise<{
    ...
    const [total, processed, pending, failed, requiringReview, avgConfidence] =
      await Promise.all([
        ...
        this.prisma.emailReply.count({
          where: { companyId, requiresReview: true },
        }),
