...
export class AIParsingService {
  ...
  async parseEmailContent(emailReply: any): Promise<ParsingResult[]> {
    try {
      ...
      // Log AI processing
      await this.logAIProcessing(emailReply.companyId, emailReply.id, 'EMAIL_PARSING', {
        content: content.substring(0, 1000) // Limit content size
      }, results, this.calculateAverageConfidence(results));

      logger.info(`Successfully parsed email ${emailReply.messageId} with ${results.length} results`);
   ...
   */
  private async extractPricesWithAI(content: string): Promise<PriceExtractionResult | null> {
    try {
      const prompt = `
...
Only return the JSON object, no other text.`;
      ...
      const response = await this.openai!.chat.completions.create({
        ...
        messages: [
          {
            role: 'system',
            content: 'You are an expert at extracting shipping quote information from emails. Return only valid JSON.'
          },
          {
            role: 'user',
            content: prompt
   ...
   */
  private async extractPricesWithRegex(content: string): Promise<PriceExtractionResult | null> {
    const result: PriceExtractionResult = {
      confidenceScore: 0
    };

    // Currency detection
    const currencyMatch = content.match(/(USD|EUR|GBP|JPY|INR|RUB|KRW|ILS|NGN|CRC|PKR|UAH|KZT|AMD|BTC)/i);
    if (currencyMatch) {
    ...
    // Price patterns
    const pricePatterns = [
      { key: 'oceanFreight', patterns: [/ocean\s*freight[:\s]*\$?([\d,]+\.?\d*)/i, /freight[:\s]*\$?([\d,]+\.?\d*)/i] },
      { key: 'baf', patterns: [/baf[:\s]*\$?([\d,]+\.?\d*)/i, /bunker\s*adjustment[:\s]*\$?([\d,]+\.?\d*)/i] },
      { key: 'caf', patterns: [/caf[:\s]*\$?([\d,]+\.?\d*)/i, /currency\s*adjustment[:\s]*\$?([\d,]+\.?\d*)/i] },
      { key: 'securityFee', patterns: [/security[:\s]*\$?([\d,]+\.?\d*)/i, /isf[:\s]*\$?([\d,]+\.?\d*)/i] },
      { key: 'documentationFee', patterns: [/documentation[:\s]*\$?([\d,]+\.?\d*)/i, /doc\s*fee[:\s]*\$?([\d,]+\.?\d*)/i] },
      { key: 'handlingCharges', patterns: [/handling[:\s]*\$?([\d,]+\.?\d*)/i, /thc[:\s]*\$?([\d,]+\.?\d*)/i] },
      { key: 'totalAmount', patterns: [/total[:\s]*\$?([\d,]+\.?\d*)/i, /grand\s*total[:\s]*\$?([\d,]+\.?\d*)/i] }
    ];
   ...
   */
  private async extractTermsWithAI(content: string): Promise<TermsExtractionResult | null> {
    try {
      const prompt = `
...
Only return the JSON object, no other text.`;
      ...
      const response = await this.openai!.chat.completions.create({
        ...
        messages: [
          {
            role: 'system',
            content: 'You are an expert at extracting shipping terms from emails. Return only valid JSON.'
          },
          {
            role: 'user',
            content: prompt
   ...
   */
  private async extractTermsWithRegex(content: string): Promise<TermsExtractionResult | null> {
   ...
   */
  async extractCurrencyInfo(content: string): Promise<CurrencyConversionResult | null> {
    try {
      const currencies = ['USD', 'EUR', 'GBP', 'JPY', 'INR', 'RUB', 'KRW', 'ILS', 'NGN', 'CRC', 'PKR', 'UAH', 'KZT', 'AMD'];
      ...
      for (const symbol of currencySymbols) {
        if (content.includes(symbol)) {
          ...
          if (symbolMap[symbol] && !foundCurrencies.includes(symbolMap[symbol])) {
   ...
   */
  async convertCurrency(from: string, to: string, amount: number): Promise<CurrencyConversionResult | null> {
    try {
   ...
   */
  private async storeParsingResult(emailReplyId: string, result: ParsingResult): Promise<void> {
    try {
  ...
  private calculateAverageConfidence(results: ParsingResult[]): number {
    ...
    const total = results.reduce((sum, result) => sum + result.confidenceScore, 0);
   ...
   */
  async validateParsingResult(resultId: string, data: {
