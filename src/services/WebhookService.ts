...
export class WebhookService {
   ...
   */
  async processWebhook(webhookId: string, payload: WebhookPayload): Promise<void> {
    try {
      ...
      if (webhookConfig.secret && payload.signature) {
        const isValid = this.verifyWebhookSignature(payload, webhookConfig.secret);
        if (!isValid) {
      ...
      switch (payload.provider.toLowerCase()) {
        case 'sendgrid':
          processed = await this.processSendGridWebhook(webhookConfig.companyId, payload);
          break;
        case 'mailgun':
          processed = await this.processMailgunWebhook(webhookConfig.companyId, payload);
          break;
        case 'ses':
          processed = await this.processSESWebhook(webhookConfig.companyId, payload);
          break;
        case 'postmark':
          processed = await this.processPostmarkWebhook(webhookConfig.companyId, payload);
          break;
        default:
          logger.warn(`Unsupported webhook provider: ${payload.provider}`);
          await this.updateWebhookError(webhookId, `Unsupported provider: ${payload.provider}`);
          return;
      }
      ...
      if (processed) {
        ...
        await this.updateWebhookError(webhookId, 'Failed to process webhook data');
      }
   ...
   */
  private async processSendGridWebhook(companyId: string, payload: WebhookPayload): Promise<boolean> {
    try {
  ...
  private parseSendGridInbound(data: any): EmailWebhookData | null {
    try {
      return {
        ...
        references: data.headers['References'] ? data.headers['References'].split(' ') : [],
        from: {
          email: data.from,
          name: data.fromname
        },
   ...
   */
  private async processMailgunWebhook(companyId: string, payload: WebhookPayload): Promise<boolean> {
    try {
   ...
   */
  private async processSESWebhook(companyId: string, payload: WebhookPayload): Promise<boolean> {
    try {
      if (payload.event === 'inbound') {
  ...
  private parseSESInbound(data: any): EmailWebhookData | null {
    try {
      return {
        ...
        references: data.mail.commonHeaders['References'] ? data.mail.commonHeaders['References'].split(' ') : [],
        from: {
   ...
   */
  private async processPostmarkWebhook(companyId: string, payload: WebhookPayload): Promise<boolean> {
    try {
      if (payload.event === 'inbound') {
   ...
   */
  private async updateWebhookError(webhookId: string, errorMessage: string): Promise<void> {
    await this.prisma.emailWebhook.update({
      where: { id: webhookId },
      data: {
   ...
   */
  async updateWebhookConfig(webhookId: string, data: {
    name?: string;
    provider?: string;
    webhookUrl?: string;
    secret?: string;
  ...
  async getWebhookStats(companyId: string): Promise<{
    ...
    const totalProcessed = webhooks.reduce((sum, w) => sum + w.totalProcessed, 0);
