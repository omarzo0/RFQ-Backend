...
export class IMAPService {
   ...
   */
  private async fetchEmails(imap: Imap, config: IMAPConfig): Promise<EmailMessage[]> {
    return new Promise((resolve, reject) => {
   ...
   */
  private shouldProcessConfig(config: { lastCheckedAt: Date | null; checkInterval: number }): boolean {
   ...
   */
  private async updateLastError(configId: string, errorMessage: string): Promise<void> {
    await this.prisma.iMAPConfiguration.update({
      where: { id: configId },
      data: {
   ...
   */
  async testConnection(config: Omit<IMAPConfig, 'id' | 'companyId'>): Promise<boolean> {
    try {
   ...
   */
  async updateIMAPConfig(configId: string, data: Partial<IMAPConfig>): Promise<IMAPConfig | null> {
    const config = await this.prisma.iMAPConfiguration.update({
      ...
      data: {
        ...
        ...(data.maxEmailsPerCheck && { maxEmailsPerCheck: data.maxEmailsPerCheck })
      }
    });
  ...
  async closeAllConnections(): Promise<void> {
    for (const [configId, imap] of this.activeConnections) {
      try {
        ...
        logger.error(`Error closing IMAP connection for config ${configId}:`, error);
      }
    }
